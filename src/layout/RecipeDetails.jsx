...
function RecipeDetails() {
    ...
    return (
        <main className="my-6 mx-auto w-11/12 rounded-md min-h-full bg-gray-800 text-white p-4">
            <h1 className="text-2xl text-center font-bold md:text-3xl tracking-wider">{name.charAt(0).toUpperCase() + name.slice(1)}</h1>
            <img
                className="my-0 mx-auto w-64 h-64  rounded-lg mt-4"
                src={image}
                ...
                </li>
                {
                    ingredients && (
                        ingredients.map((ingredient, index) => (
                            <li
                                className="text-lg ml-10 md:text-xl md:ml-0"
                                key={index}
                            >- {ingredient}</li>
                        ))
                    )
                            ...
                            loader ? (
                                <>
                                    ...
                                    <div className="flex items-center justify-center mt-2 justify-evenly">
                                        <button className="text-lg cursor-pointer duration-500 sm:active:bg-gray-600 hover:bg-gray-600 rounded p-2 w-20 text-center" onClick={() => deleteRecipe(_id)}>Tak</button>
                                        <button className="text-lg cursor-pointer duration-500 sm:active:bg-gray-600 hover:bg-gray-600 rounded p-2 w-20 text-center" onClick={() => setPopUp(false)}>Nie</button>
                                    </div>
                                )
                        }
                    </section>
                ) : (<button className="border-2 p-2 border-dashed rounded-md duration-500 hover:bg-gray-600 mt-8 md:mb-4 md:ml-4" onClick={() => setPopUp(true)}>Usuń przepis</button>)
            }
        </main >
    )
}
